    ...
    >
      <view class="u-steps__item" :class="['u-steps__item--' + direction]" v-for="(item, index) in list" :key="index">
        <view
          ...
          v-if="mode == 'number'"
          :style="{
            backgroundColor: current < index ? '#ededed' : activeColor,
            borderColor: current < index ? unActiveColor : activeColor,
          }"
        >
          <text
            v-if="current < index || true"
            :class="{
        ...
        <view class="u-steps__item__line" :class="['u-steps__item__line--' + mode]" v-if="index < list.length - 1">
          <view class="m-line" :class="{ active: index <= current }"></view>
          <!-- <u-line :direction="direction" length="100%" :hair-line="false" :color="index <= current ? activeColor : unActiveColor" border-style="dashed"></u-line> -->
        </view>
      </view>
    </view>
  </view>
...
.m-line {
  height: 6rpx;
  background-image: url(~@/static/dotted-unactive.png);
