  ...
  <view class="">
    <movable-area class="u-swipe-action" :style="{ backgroundColor: bgColor }">
      ...
      >
        ...
        <view class="u-swipe-del" v-show="showBtn" @tap.stop="onRemove">
          <image src="@/static/crash.png" class="crash-icon" />
          <!-- <Icon v-if="item.icon" :name="item.icon" /> -->
          <!-- <view class="u-btn-text" v-else>{{ item.text }}</view> -->
        </view>
      </movable-view>
    </movable-area>
  </view>
...
export default {
  ...
  computed: {
    ...
    allBtnWidth() {
      return uni.upx2px(this.btnWidth) * 1
    },
    btnStyle() {
      return (style) => {
        let css = {}
