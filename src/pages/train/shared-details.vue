...
<template>
  <view class="content">
    ...
    <template v-if="!!details">
      <u-swiper
        :list="get(details, 'images', []).map((item) => `http://zhschool.qixiuu.com${item}`)"
        :height="642"
        mode="number"
      ...
      <template v-if="currentNav === 0">
        ...
        <Card v-if="get(details, 'money_detail_images')">
          ...
          <image
            class="w-full"
            :src="`http://zhschool.qixiuu.com${get(details, 'money_detail_images')}`"
            mode="widthFix"
          />
        </Card>
      ...
      <template v-if="currentNav === 1">
        ...
        <image
          ...
          mode="widthFix"
          class="w-full"
          :src="`http://zhschool.qixiuu.com${item}`"
        />
        <!-- </Card> -->
      </template>
      ...
      <view class="footer fixed flex" v-if="isSharePage">
        <u-button shape="square" class="w-full" type="primary" @click="navigateTo('share-download')">下载智慧海院APP</u-button>
      </view>
